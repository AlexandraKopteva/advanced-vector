...
public:
	...
	iterator Emplace(const_iterator pos, Args&&... args)
	{
		...
		auto distance = static_cast<size_t>(pos - begin());
		...
		if (size_ == Capacity())
		{
			...
			Uninitialized(data_.GetAddress(), distance, new_data.GetAddress());
			Uninitialized(data_.GetAddress() + distance, size_ - distance, new_data + distance + 1);
		...
		else
		{
			...
			//инициализируем её размещающим new остальные элементы переносим на один вправо
			new (end()) T(std::move(data_[size_ - 1]));
			std::move_backward(begin() + distance, end() - 1, end());
			data_[distance] = std::move(new_data);
		}
...
private:
	...
	void Uninitialized(T* data, int pos, T* new_data)
	{
